...
benchmark_data['size'] = benchmark_data['name'].str.extract('Gol/(\d+)x\d+x100-1-12').apply(pd.to_numeric)
# benchmark_data['cpu_cores'] = benchmark_data['name'].str.extract('Filter/\d+_workers-(\d+)').apply(pd.to_numeric)

print(benchmark_data)

# Plot a bar chart.
ax = sns.barplot(data=benchmark_data, x='size', y='time')


# Set descriptive axis lables.
ax.set(xlabel='Image width and height', ylabel='Time taken (s)')
